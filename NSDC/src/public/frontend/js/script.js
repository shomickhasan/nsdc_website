  ...
  THEMEMASCOT.isLTR = {
	...
	function headerStyle() {
		if($('.main-header').length){
			...
			let urlWithoutRoot = path + queryString + hash;

			console.log(urlWithoutRoot);
			if(!(urlWithoutRoot === '/otithi/book_tour.php')){
				if (windowpos > 100) {
   ...
   /* if ($('.testimonial-carousel').length) {
        $('.testimonial-carousel').owlCarousel({
            rtl: THEMEMASCOT.isRTL.check(), // Check for RTL if needed
            loop: true,
            margin: 30,
            nav: false,
            dots: false,
            ...
            responsive: {
                0: {
                    ...
                    margin: 10, // Smaller margin for small screens
                },
                600: {
                    items: 1, // Two items for medium screens and above
...
$('.training-carousel-two .owl-carousel').owlCarousel({
    stagePadding: 50,
    loop: true,
    margin: 10,
    nav: true,
    ...
    navContainer: '.training-carousel-two .custom-nav',
    responsive:{
        0:{
            items: 1
        },
...
// Add or remove the sticky class to the header when you scroll
...
document.getElementById('expand-toggle').addEventListener('click', function() {
	...
	itinerary.classList.toggle('expand');
});
